...
def similarity(df1, df2):
    ...
    df2 = df2[:min_len]
    # Pearson Correlation
    pearson_corr = np.corrcoef(df1, df2)[0, 1]
    pearson_corr2, p_value = pearsonr(df1, df2)
    # Cosine Similarity
    cosine = np.dot(df1, df2) / (np.linalg.norm(df1) * np.linalg.norm(df2))
    return pearson_corr, pearson_corr2, p_value, cosine
...
def plot_similarity_scores(pair_labels, pearson_list, cosine_list, save=True):
    ...
    ax.set_title("Form Similarity of Paired SMP Profiles (aligned)")
    ...
    for xi, label in zip(x, pair_labels):
        ...
        ax.text(xi, -0.05, label, ha='center', fontsize=9)
...
if __name__ == "__main__":
    ...
    smp_profiles = load_all_smp_profiles(pnt=False, aligned="first")  # load all SMP profiles
    ...
    for df8, name8, df20, name20 in paired_profiles:
        ...
        pearson, pearson2, p_val, cosine = similarity(force8, force20)

        print(f"{name8} vs {name20}:")
        print(f"  Pearson Correlation: {pearson:.4f}")
        print(f"  Pearson Correlation2: {pearson2:.4f} (p-value: {p_val:.4e})")
    ...
    plot_similarity_scores(pair_labels, pearson_list, cosine_list, save=False)
    #load_all_smp_profiles(pnt=False, aligned="pairs")  # load all SMP profiles