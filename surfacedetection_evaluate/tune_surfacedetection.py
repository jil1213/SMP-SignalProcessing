...
def plot_delta_error(surface_ini_all, surface_old_all, surface_new_all, folder_path):
        ...
        #Shapiro-Wilk Test to test normality of the distributions
        from scipy.stats import shapiro

        stat_old, p_old = shapiro(delta_old)
        stat_new, p_new = shapiro(delta_new)

        print(f"Shapiro-Wilk Test (existing method): p = {p_old:.4e}")
        print(f"Shapiro-Wilk Test (new method):      p = {p_new:.4e}")
        print("not normally distributed, when p < 0.05")

        # Mann Whitney U Test
        from scipy.stats import mannwhitneyu
        stat_mw, p_mw = mannwhitneyu(delta_old, delta_new, alternative='two-sided')
        print(f"Mann-Whitney U Test: Statistic = {stat_mw:.4f}, p-value = {p_mw:.4e}")
        ...
        # Highlight Baselinie
        plt.axhline(0, linewidth=2.5, color='black', linestyle='-', zorder=3)
        plt.plot(delta_old, label="Existing method", marker='.', linestyle='None', markersize=5)
        plt.plot(delta_new, label="New method ", marker='.', linestyle='None', markersize=5)
        # Dummy handle for text explanation
        #plt.plot([], [], ' ',label="Note:\nNegative values mean surface is detected earlier\nthan in manually ini file.\n delta = surface - surface_ini")
        plt.xlabel("Profile Index", fontsize=14)
        plt.ylabel("Deviation to manual reference (mm)", fontsize=14)
        ...
        plt.ylim(-7.5, 7.5) #zoomed range for better visibility of small errors
        plt.legend()
        plt.grid()
        plt.tight_layout()
        ...
        plt.boxplot([delta_old, delta_new], labels=["Existing method", "New method"], widths=0.4, medianprops=dict(linewidth=2.5))
        plt.tick_params(axis='x', labelsize=14)  # Größe der Boxplot-Labels
        plt.ylabel("Deviation to manual reference (mm)", fontsize=14)
