...
from code_automated_correlation.a_automated_processing import align_profiles
...
def plot_comparison(mean_profiles, date, target_dir, save): 
...
def calc_mean(smp_profiles, target_dir=Path("output/visualizations_mean"), save=False, buffer=True, plot=True):
    """
    ...
    for date in [1, 2]:
        ...
        for velocity in [20, 8]:
            ...
            #make sure that all profiles have the same length (trim to shortest of all)
            min_len = min(len(df) for df in subset.values())
            subset = {name: df.iloc[:min_len].copy() for name, df in subset.items()}
            ...
            if plot == True:
                plot_mean(distance, mean_force, std, date, velocity, target_dir, save)

            #save in one array for each velocity
            mean_profiles[velocity] = {"distance": distance,"mean": mean_force,"std": std}

        if plot == True:
            plot_comparison(mean_profiles, date, target_dir, save)

        all_mean_profiles[date] = mean_profiles

    return all_mean_profiles
